...
BLR_ICE_X_PS <- function(dat){
  ...
  ############################# Stage 2 ###################################
  X = cbind(1,dat$A1,dat$A2,dat$X_11,dat$X_21,dat$X_3,dat$L1,dat$X_12,dat$X_22,dat$L2)   
  ...
  Sigma_1_result <- c()
  Delta_1_result <- matrix(nrow=0,ncol=7)
  ...
  for(i in 1:length(sigma_2_sub)){
    ...
    newdata$a2_0 <- 0
    newdata$a2_1 <- 1
    X_a2_0 <- as.matrix(cbind(1,newdata[,c("A1","a2_0","X_11","X_21","X_3","L1","X_12","X_22","L2")]))
    X_a2_1 <- as.matrix(cbind(1,newdata[,c("A1","a2_1","X_11","X_21","X_3","L1","X_12","X_22","L2")]))
    # find Y_a2_0
    ...
    X <- as.matrix(cbind(1,data_aug$A1,data_aug$a2,data_aug$X_11,
                         data_aug$X_21,data_aug$X_3,data_aug$L1))
    y <- data_aug$Y_hat
...
for (i in 1:M){
  ...
  # Tan 2019 Section 3.1 Data-generating mechanism
  ...
  if((true_ATE_Tan_2019 <= upper_CI) & (true_ATE_Tan_2019 >= lower_CI)){
...
# Percentage Bias
bias_BLR <- mean(est_ATE)-true_ATE_Tan_2019
percentage_bias_BLR <- abs(bias_BLR/true_ATE_Tan_2019)
paste("(BLR_Tan_X_PS) Percentage Bias: ", percentage_bias_BLR)
...
paste("(BLR_Tan_X_PS) Credible Interval Coverage: ", credible_interval_coverage_BLR)

# RMSE
BLR_RMSE <- sqrt(mean((est_ATE - true_ATE_Tan_2019)^2))
paste("(BLR_Tan_X_PS) RMSE: ", BLR_RMSE)

# MCSE
MCSE <- sd(est_ATE)
paste("(BLR_Tan_X_PS) MCSE: ", MCSE)

# AESE
AESE <- mean(sd_ATE)
paste("(BLR_Tan_X_PS) AESE: ", AESE)
