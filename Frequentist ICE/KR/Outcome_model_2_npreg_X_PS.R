...
# Percentage Bias
bias_npreg <- mean(est_ATE)-true_ATE_Zhu
percentage_bias_npreg <- abs(bias_npreg/true_ATE_Zhu)
paste("(freq_npreg_Zhu_X_PS) Percentage Bias: ", percentage_bias_npreg)

# MCSD
MCSD <- sd(est_ATE)
paste("(freq_npreg_Zhu_X_PS) MCSD: ", MCSD)

# RMSE
npreg_RMSE <- sqrt(mean((est_ATE - true_ATE_Zhu)^2))
paste("(freq_npreg_Zhu_X_PS) RMSE: ", npreg_RMSE)


#credible_interval_coverage_npreg <- coverage_num/M
#paste("(freq_npreg_Zhu_X_PS) Credible Interval Coverage: ", credible_interval_coverage_npreg)
