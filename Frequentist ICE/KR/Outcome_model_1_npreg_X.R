...
paste("(freq_npreg_Linear_X) Percentage Bias: ", percentage_bias_npreg)

# MCSD
MCSD <- sd(est_ATE)
paste("(freq_npreg_Linear_X) MCSD: ", MCSD)

# RMSE
npreg_RMSE <- sqrt(mean((est_ATE - true_ATE_Linear)^2))
paste("(freq_npreg_Linear_X) RMSE: ", npreg_RMSE)


#credible_interval_coverage_npreg <- coverage_num/M
#paste("(freq_npreg_Linear_X) Credible Interval Coverage: ", credible_interval_coverage_npreg)
