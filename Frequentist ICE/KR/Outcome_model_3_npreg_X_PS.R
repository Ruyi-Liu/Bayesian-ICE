...
paste("(freq_npreg_Tan_X_PS) Percentage Bias: ", percentage_bias_npreg)

# MCSD
MCSD <- sd(est_ATE)
paste("(freq_npreg_Tan_X_PS) MCSD: ", MCSD)

# RMSE
npreg_RMSE <- sqrt(mean((est_ATE - true_ATE_Tan)^2))
paste("(freq_npreg_Tan_X_PS) RMSE: ", npreg_RMSE)


#credible_interval_coverage_npreg <- coverage_num/M
#paste("(freq_npreg_Tan_X_PS) Credible Interval Coverage: ", credible_interval_coverage_npreg)
