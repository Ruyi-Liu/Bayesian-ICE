...
# Percentage Bias
bias_RF <- mean(est_ATE)-true_ATE_Linear
percentage_bias_RF <- abs(bias_RF/true_ATE_Linear)
paste("(freq_RF_Linear_X_PS) Percentage Bias: ", percentage_bias_RF)

# MCSD
MCSD <- sd(est_ATE)
paste("(freq_RF_Linear_X_PS) MCSD: ", MCSD)

# RMSE
RF_RMSE <- sqrt(mean((est_ATE - true_ATE_Linear)^2))
paste("(freq_RF_Linear_X_PS) RMSE: ", RF_RMSE)
...
paste("(freq_RF_Linear_X_PS) Credible Interval Coverage: ", credible_interval_coverage_RF)
