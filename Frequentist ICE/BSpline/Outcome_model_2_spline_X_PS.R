...
for (i in 1:M){
  ...
  if((true_ATE_Zhu <= upper_CI) & (true_ATE_Zhu >= lower_CI)){
...
# Percentage Bias
bias_Bspline <- mean(est_ATE)-true_ATE_Zhu
percentage_bias_Bspline <- abs(bias_Bspline/true_ATE_Zhu)
paste("(freq_spline_Zhu_X_PS) Percentage Bias: ", percentage_bias_Bspline)

# MCSD
MCSD <- sd(est_ATE)
paste("(freq_spline_Zhu_X_PS) MCSD: ", MCSD)

# RMSE
Bspline_RMSE <- sqrt(mean((est_ATE - true_ATE_Zhu)^2))
paste("(freq_spline_Zhu_X_PS) RMSE: ", Bspline_RMSE)
...
paste("(freq_spline_Zhu_X_PS) Credible Interval Coverage: ", credible_interval_coverage_Bspline)
